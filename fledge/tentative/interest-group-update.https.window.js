...
const makeTestForUpdate = ({
  ...
  subsetTest(promise_test, async test => {
    ...
    // When checking the render URL, both the old and new versions might exist
    // in the interest group, so this test deletes the old one to ensure a
...
// * `failedUpdateGroup`: Receives an invalid update, and will continue to throw errors until the update occurs (which shouldn't happen).
// * `successUpdateGroup`: A hard-coded interest group that receives a update and will signal the change by throwing an error.
...
// It then will run one final auction. If there's still no winner, it can infer that `failedUpdateGroup` would have
